# Роль Ansible: nfs_server - NFS файловый сервер с аутентификацией Kerberos

Данный репозиторий содержит Для Демонстрации автоматизированную конфигурацию для развертывания и управления файловым сервером на базе NFS с интеграцией в домен Active Directory и поддержкой безопасной аутентификации через Kerberos на базе **ALT Linux** (семейство `apt-rpm`). Роль обеспечивает автоматическое создание доменных пользователей и групп, настройку экспортируемых ресурсов с разграничением прав доступа, присоединение сервера к домену и конфигурацию службы NFS для работы в защищенной корпоративной среде.
...
| **Зависимости** | Предварительно развернутый Samba AD DC с работающим Kerberos |
...
├── nfs_server.yaml             # Плейбук вызова роли NFS-сервера
├── inventory/
...
└── roles/
    └── nfs_server/             # Каталог роли NFS-сервера
        ├── tasks/
        │   ├── main.yml        # Точка входа и распределение задач
        │   ├── nfs_ad_prerare.yml  # Подготовка AD: DNS, пользователи, группы
        │   └── nfs_install.yml # Установка и настройка NFS-сервера
        ├── templates/
        │   └── exports.j2      # Шаблон конфигурации экспорта /etc/exports
        ├── files/
        │   └── nfs             # Файл настройки SECURE_NFS для sysconfig
        ├── handlers/
        │   └── main.yml        # Обработчики (при необходимости)
        ├── vars/
...
- **`inventory/group_vars/all/all.yml`** содержит параметры, используемые ролью:
  ...
  - `network_subnet`: подсеть клиентов для экспорта (`192.168.100.0/24`)
  - `spec_nfs_gr1`: имя специальной доменной группы для разграничения доступа
  - `nfs_users`: словарь доменных пользователей с параметрами (имя, пароль, отображаемое имя, email)
  - `nfs_shares`: конфигурация экспортируемых каталогов с путями
  - `main_nfs_options`: базовые опции экспорта (`rw,no_subtree_check`)
  - `secure_nfs_options`: опции безопасности Kerberos (`,sec=krb5:krb5i:krb5p`)
  - `nfs_server`: триггер включения роли (`true`/`false`)
...
Роль `nfs_server` выполняет настройку по следующему сценарию:
...
2. **Установка и настройка NFS-сервера** (выполняется на хостах группы `file_servers`)
   - Обновляет кеш пакетов и устанавливает `rpcbind`, `nfs-clients`, `nfs-server`
   - Разворачивает файл `/etc/sysconfig/nfs` с параметром `SECURE_NFS=yes` для включения поддержки Kerberos
   ...
   - Создает каталоги для экспорта с указанными владельцами и группами
   - Настраивает права доступа для каждого ресурса согласно конфигурации:
     - `root`: общий корневой каталог с битом setgid
     - `NOTadmins`: закрытый ресурс только для администраторов
     - `work`: рабочие каталоги для пользователей домена
     - `spec_GR1`: ресурс для специальной группы
   - Разворачивает конфигурационный файл `/etc/exports` с параметрами экспорта для подсети и опциями безопасности Kerberos
   - Получает Kerberos-принципал для службы NFS через `net ads keytab add nfs`
   - Включает и запускает службы `rpc-gssd.service` и `nfs-server`
   - Применяет экспорт ресурсов командой `exportfs -vra`
...
# Запуск только роли NFS-сервера
ansible-playbook -i inventory/inventory.yaml nfs_server.yaml \
  --vault-password-file ./va_pa

# Запуск через главный плейбук (с учетом триггера nfs_server)
ansible-playbook -i inventory/inventory.yaml main.yaml \
  --vault-password-file ./va_pa
```

> Переменные `nfs_server`, `spec_nfs_gr1`, `nfs_users`, `nfs_shares`, `secure_nfs_options` можно переопределить через `--extra-vars` или в `group_vars`.
...
5. Пароли доменных пользователей, указанные в `nfs_users`, должны соответствовать политике сложности паролей домена.
...
7. После применения роли проверьте доступ к ресурсам командой `showmount -e hostname` и убедитесь в корректной работе монтирования с опцией `sec=krb5` через `mount -t nfs4 -o sec=krb5 server:/path /mnt`.
8. Регулярно проверяйте логи NFS `/var/log/messages` и `/var/log/krb5kdc.log` на предмет ошибок аутентификации или доступа.
9. При изменении прав доступа к экспортируемым ресурсам обновляйте конфигурацию через Ansible, а не вручную, чтобы избежать рассинхронизации состояния.
10. Убедитесь, что время на всех узлах синхронизировано через Chrony, так как Kerberos критичен к расхождению времени.